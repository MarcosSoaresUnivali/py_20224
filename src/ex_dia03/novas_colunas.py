# %%
# Import data from csv...


import pandas as pd
import numpy as np

df = pd.read_csv("customers.csv", sep=';')
df
# %%
df["Points_double"] = df["Points"] * 2
df
# %%
df["Points_ratio"] = df["Points_double"] / df["Points"]
df
# %%
df["Constante"] = None
df
# %%
df["Points_log"] = np.log(df["Points"])
df
# %%
df["Name_Upper"] = df["Name"].str.upper()
df
# %%

def get_first(nome:str):
    return nome.split("_")[0]


df["Name_1st"] = df["Name"].apply( get_first )
df
# %%

get_f = lambda nome: nome.split("_")[0]
get_f("mps_1")
# %%
df["Name_f"] = df["Name"].apply( lambda x: x.split("_")[0] )
df
# %%

def intervalo_pontos(pontos):
    if pontos < 2500:
        return "baixo"
    elif pontos < 3500:
        return "medio"
    else:
        return "alto"
    
df["Faixa_pts"] = df["Points"].apply(intervalo_pontos)
df

# %%
df["UID_3"] = df["UUID"].apply(lambda x: x[-3:])
df
# %%
df["UID_m3"] = df["UUID"].apply(lambda x: x[:3])
df
# %%

data = {
    "nome": ["Teo","Nah","Maria","Lara"],
    "recencia": [1,30,10,45],
    "valor": [100,2000,15,500],
    "frequencia": [2,5,1,15]
}

df_crm = pd.DataFrame(data)
df_crm
# %%

# Se  "recencia" <= 10
#     10
# Se  "recencia" > 10 e < 30
#     5
# Se  "recencia" > 30
#     0
# ==========================
# Se  "valor" > 1000
#     10
# Se  "valor" < 1000 e > 100
#     5
# Se  "valor" < 100
#     0
# ==========================
# Se  "frequencia" > 10
#     10
# Se  "frequencia" < 10 e > 5
#     5
# Se  "frequencia" < 5
#     0
# ==========================
# nota_final = soma
# ==========================

def rfv(row):
    nota = 0

    if row['recencia'] <= 10:
        nota += 10
    elif 10 < row['recencia'] <= 30:
        nota += 5
    elif row['recencia'] > 30:
        nota += 0

    if row['valor'] > 1000:
        nota += 10
    elif 100 <= row['valor'] < 1000:
        nota += 5
    elif row['valor'] < 100:
        nota += 0

    if row['frequencia'] > 10:
        nota += 10
    elif 5 <= row['frequencia'] < 10:
        nota += 5
    elif row['frequencia'] < 5:
        nota += 0

    return nota


df_crm["nota"] = df_crm.apply(rfv, axis=1)
df_crm

# %%
